<div class="card">
    <form class="form-horizontal" method="post" action="<?= base_url('tebakgambar/update') ?>" enctype="multipart/form-data">
        <input type="hidden" name="id" value="<?= $data[0]->id_tebakgambar ?>">
        <div class="card-body">
            <h4 class="card-title">Edit Kamus</h4>
            <div class="form-group row">
                ...
                <div class="col-md-9">
                    <select class="select2 form-control custom-select" name="id_kamus" style="width: 100%; height:36px;">
                        <?php $datas = $this->m_master->kamus();
                        foreach ($datas as $dt) {
                            if ($data[0]->id_kamus == $dt->id_kamus) {
                                echo "<option value=$dt->id_kamus selected>$dt->english</option>";
                            } else {
